...
parse_packet_wlan(unsigned char** buf, int len, struct packet_info* p)
{
	...
	DEBUG_PRINT("before parse 80211 len: %d\n", len);
...
parse_prism_header(unsigned char** buf, int len, struct packet_info* p)
{
	...
	*/
	...
	DEBUG_PRINT("devname: %s\n", ph->devname);
	DEBUG_PRINT("signal: %d -> %d\n", ph->signal.data, p->phy_signal);
	DEBUG_PRINT("rate: %d\n", ph->rate.data);
	DEBUG_PRINT("rssi: %d\n", ph->rssi.data);
...
get_radiotap_info(struct ieee80211_radiotap_iterator *iter, struct packet_info* p)
{
	...
	case IEEE80211_RADIOTAP_FLAGS:
		/* short preamble */
		DEBUG_PRINT("[flags %0x", *iter->this_arg);
		if (*iter->this_arg & IEEE80211_RADIOTAP_F_SHORTPRE) {
			p->phy_flags |= PHY_FLAG_SHORTPRE;
			DEBUG_PRINT(" shortpre");
		}
	...
	case IEEE80211_RADIOTAP_RATE:
		...
		DEBUG_PRINT("[rate %0x]", *iter->this_arg);
...
#define IEEE80211_CHAN_G \
	...
	case IEEE80211_RADIOTAP_DBM_ANTNOISE:
		DEBUG_PRINT("[noi %0x]", *(signed char*)iter->this_arg);
		// usually not present
		//p->phy_noise = *(signed char*)iter->this_arg;
		break;
	case IEEE80211_RADIOTAP_ANTENNA:
		DEBUG_PRINT("[ant %0x]", *iter->this_arg);
		break;
	case IEEE80211_RADIOTAP_DB_ANTSIGNAL:
		DEBUG_PRINT("[snr %0x]", *iter->this_arg);
		// usually not present
	...
	case IEEE80211_RADIOTAP_MCS:
		...
		known = *iter->this_arg++;
		flags = *iter->this_arg++;
		DEBUG_PRINT("[MCS known %0x flags %0x index %0x]", known, flags, *iter->this_arg);
		...
		DEBUG_PRINT(" %s %s", ht20 ? "HT20" : "HT40", lgi ? "LGI" : "SGI");

		p->phy_rate_idx = 12 + *iter->this_arg;
		p->phy_rate_flags = flags;
		p->phy_rate = mcs_index_to_rate(*iter->this_arg, ht20, lgi);

		DEBUG_PRINT(" RATE %d ", p->phy_rate);
		break;
	default:
		DEBUG_PRINT("UNKNOWN RADIOTAP field %d", iter->this_arg_index);
		break;
	}
}
...
parse_radiotap_header(unsigned char** buf, int len, struct packet_info* p)
{
	...
	if (err) {
		DEBUG_PRINT("malformed radiotap header (init returns %d)\n", err);
		return -1;
	}

	DEBUG_PRINT("Radiotap: ");
...
parse_80211_header(unsigned char** buf, int len, struct packet_info* p)
{
	...
	if (WLAN_FRAME_IS_DATA(fc)) {
		...
		DEBUG_PRINT("DATA NAV %d\n", p->wlan_nav);
		p->wlan_seqno = le16toh(wh->seq);
		DEBUG_PRINT("DATA SEQ %d\n", p->wlan_seqno);
		...
		bssid = wh->addr3;
		p->wlan_seqno = le16toh(wh->seq);
		DEBUG_PRINT("MGMT SEQ %d\n", p->wlan_seqno);

		if (fc & WLAN_FRAME_FC_RETRY)
			p->wlan_retry = 1;
	} else {
		DEBUG_PRINT("!!!UNKNOWN FRAME!!!");
		return -1;
	}
